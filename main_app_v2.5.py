...
def get_db_url(path=""):
    # Bu script sadece lisanslar dalı ile ilgilenir
    return f"{FIREBASE_DATABASE_URL}/lisanslar/{path}.json?key={FIREBASE_API_KEY}"

# --- LİSANS VE KULLANICI FONKSİYONLARI ---

# --- DÜZELTME BAŞLANGICI: check_license fonksiyonu ---
# Hata yakalama bloğu daha spesifik hale getirildi.
def check_license(license_key):
    """Lisansı doğrular ve lisans verisini döndürür."""
    print(ACCENT_COLOR + "Lisans durumu doğrulanıyor, lütfen bekleyin...")
    try:
        # Timeout süresini 7 saniyeye çıkaralım
        response = requests.get(get_db_url(license_key), timeout=7)
        
        # Spesifik HTTP hatalarını yakala
        if response.status_code == 401:
            print(ERROR_COLOR + "HATA: Sunucuya erişim yetkiniz yok (401).")
            print(NORMAL_COLOR + "  Firebase 'Rules' ayarlarınızı kontrol edin.")
            return None
        if response.status_code == 404 or response.text == "null":
            print(ERROR_COLOR + "HATA: Geçersiz veya bulunamayan lisans anahtarı (404).")
            return None
        
        # Diğer 4xx/5xx sunucu hataları için
        response.raise_for_status() 
        
        data = response.json()

        # Son Görülme Güncellemesi (Hata verirse bile program devam etsin)
        try:
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            update_url = get_db_url(f"{license_key}/son_gorulme")
            requests.put(update_url, data=json.dumps(timestamp), timeout=2) 
        except Exception:
            pass # Bu kritik değil, başarısız olursa sessizce geç

        # Lisans durumu kontrolleri
        ...
        print(ERROR_COLOR + "HATA: Lisans sunucusu zaman aşımına uğradı (Timeout).")
        print(NORMAL_COLOR + "  İnternet bağlantınız yavaş olabilir veya sunucu meşgul.")
        return None
    except requests.exceptions.RequestException as e:
        # Bu, 'bağlanılamadı' hatasının gerçek versiyonudur
        print(ERROR_COLOR + "AĞ HATASI: Lisans sunucusuna bağlanılamadı.")
        print(NORMAL_COLOR + f"  Detay: {e}")
        return None
    except Exception as e:
        # Diğer tüm beklenmedik hatalar (JSON çözme hatası vb.)
        print(ERROR_COLOR + f"Lisans kontrolünde beklenmedik bir hata oluştu: {e}")
        return None
# --- DÜZELTME SONU ---
...
def run_reward_process(license_data):
    ...
    try:
        ...
        if 1 <= secim <= len(num_list):
            ...
            main_program_logic(phone_number) 
        else:
            print(ERROR_COLOR + "  Geçersiz seçim.")
    except ValueError:
